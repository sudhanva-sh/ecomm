...
public class OrderCreatedListener {
    ...
    public void handleOrderEvent(OrderCreatedEvent event, org.apache.kafka.common.header.Headers headers) {


        headers.forEach(header ->
                logger.info(String.format( "Headers %s : %s ",
                        header.key(), new String(header.value())
                ))
        );
//        logger.info(String.format("Received from Order Service: %s", headers));
